...
- `experiments/plot_results.py`: regenerates the paper figure and summary CSV.
...
`results/submission_readiness.csv`; it is intentionally report-only by default
because the current honest draft can be internally reproducible while still
waiting on a public anonymous URL. Once the anonymous.4open.science link exists,
`finalize_anonymous_url.py` updates the manuscript and checklist in one step.
