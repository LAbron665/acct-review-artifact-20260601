...
def audit_readiness(paper_text: str, root: Path) -> list[dict[str, str]]:
    ...
    if publish_report.get("status") == "PASS" and publish_report.get("dry_run") is True:
        add_row(rows, "publish_dry_run", "PASS", "publish helper dry-run report is PASS", "None")
    else:
        add_row(rows, "publish_dry_run", "NOT_READY", "publish helper dry-run report is absent or not PASS", "Run publish_anonymous_release_repo.py --dry-run with the intended fresh remote.")
