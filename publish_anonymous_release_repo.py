...
def add_git_remote_push(commands: list[dict[str, object]], repo: Path, remote: str, branch: str) -> None:
    ...
    commands.append(run(["git", "push", "-u", "origin", branch], cwd=repo))
...
def main() -> None:
    ...
    if not failures and not args.dry_run:
        if args.remote:
            add_git_remote_push(commands, repo=repo, remote=args.remote, branch=args.branch)
        else:
